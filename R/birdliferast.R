...
birdliferast<-function(speclist=NULL,inpath=NULL,outpath=NULL,rastg=NULL,season=NULL,inproj=NULL,outproj=NULL){
    ...
    results<-foreach(i=1:length(speclist),.combine=rbind,.errorhandling=c('remove')) %dopar% {
        cat(as.character(speclist[i]))
        cat("\n")
        binomial<-str_split_fixed(speclist[i],".shp",3)[,1]
        filep<-paste(inpath,"/",speclist[i],sep="")
        tmpshp<-readOGR(filep,binomial,stringsAsFactors=FALSE,verbose=FALSE)
        ...
        data.frame(value=as.numeric(na.exclude(unlist(cats))),presabs=1,binomial)  # create data frame with presabs
